...
# 모델 불러오기
model = Crowd_locator()
model.load_state_dict(torch.load('./exp/04-05_14-45_JHU_VGG16_FPN/ep_25_F1_0.000_Pre_0.000_Rec_0.000_mae_392.5_mse_1055.0.pth'))
model.eval()
...
# 입력한 손글씨 이미지 불러오기
for i in range(10):
    img_path = '../ProcessedData/JHU/images/0006.jpg'.format(i)
    img = Image.open(img_path).convert('L') # 이미지를 그레이스케일로 변환
    img = img.resize((28, 28)) # 이미지 크기를 모델이 학습한 크기(28x28)로 조정
    img_array = np.array(img).reshape((1, 1, 28, 28)) # 모델 입력 형태로 이미지 변환
    img_tensor = torch.tensor(img_array, dtype=torch.float32) # 이미지를 tensor로 변환
    ...
    axs[i].imshow(img)
    axs[i].set_title(f"Prediction: {pred}")
plt.tight_layout()
plt.show()
